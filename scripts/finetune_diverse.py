...
def load_diverse_dataset(dataset_name: str, dataset_split: str = "train") -> Tuple[List[Dict], List[Dict]]:
    """Load diverse_1k dataset from HuggingFace and convert to Phase-0/Phase-1 examples.
    
    Uses the 'legal' field to determine positive/negative examples.
    """
    print(f"Loading dataset from HuggingFace: {dataset_name} (split: {dataset_split})...")
    ...
    for row in tqdm(hf_dataset, desc="Processing examples"):
...
def train(
    ...
    # Load dataset
    phase0_data, phase1_data = load_diverse_dataset(dataset_name, dataset_split)
